...
const getSalonById = asyncfunhandler(async (req, res, next) => {
  const salonbyId = await Salonregistered.findById(req.params.salonId)
    .select("-location -__v")
    .populate("owner");
  res.status(200).json({
    status: "success",
...
const findSalonWith_in = asyncfunhandler(async (req, res, next) => {
  ...
  const response = await Salonregistered.find({
...
const getServices = asyncfunhandler(async (req, res) => {
  ...
  if (req.params.salonID) {
    allServices = await Services.find({
      servicesCreatedBy: req.params.salonID,
    }).populate("servicesCreatedBy");
  } else {
    const currentSalon = await Salonregistered.find({ owner: req.user._id });
    allServices = await Services.find({
...
const handleRegistered = asyncfunhandler(async (req, res, next) => {
  ...
  const registeredSalon = await Salonregistered.create({
    ...
    location: {
      coordinates: [
        Number(req.body.location.split(",")[0]),
        Number(req.body.location.split(",")[1]),
      ],
    },
...
// Edit services [price and images];

const handleEditServices = asyncfunhandler(async (req, res, next) => {
...
const getsalonbyownid = asyncfunhandler(async (req, res, next) => {
  const salonbyowner = await Salonregistered.find({ owner: req.user._id });
  if (salonbyowner.length > 0) {
    res.status(200).json({
      isregistered: true,
      salonbyowner,
    });
