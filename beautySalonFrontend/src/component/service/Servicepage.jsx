  ...
  return (
    <div className="Service-infos  min-h-80  mt-4">
      ...
      <div className="service-content">
        {isSalonnameDisplay && (
          <div className="servicename  mt-4 mb-4">
            <span className="font-bold text-lg">
              {" "}
              {item?.servicesCreatedBy?.salonName}
            </span>{" "}
            | <span className="text-sm"> {item?.servicesCreatedBy?.phone}</span>
          </div>
        )}
        <div
          className={`${textSize} servicename  mt-2 mb-2    font-bold text-pink-700`}
        >
          {item?.serviceName}
        </div>
        {ispriceDisplay && (
          <div className="servicename  mt-2 mb-2   text-xl font-bold text-green-600">
            {item?.price}
          </div>
        )}
        <div className="book-now w-3/5 mt-5 mb-4 ">
          {ispriceDisplay && (
            <Link
              to={`${
                state === true
                  ? `/bookingService/${item?._id}/${item?.serviceName}/${item.price}/${item?.servicesCreatedBy?._id}/${item?.servicesCreatedBy?.salonName}`
                  : "/login"
              }`}
...
function Servicepage() {
  ...
  const salon_with_in_range = JSON.parse(
    localStorage.getItem("salon_with_inrange")
  );
  const find_all_service_withInRange = async () => {
    try {
      ...
      localStorage.setItem(
        "servceswithinrange",
        JSON.stringify(servces_with_in_range)
      );
      const unique_Array_with_in_range = servces_with_in_range.reduce(
        (acc, currentv) => {
          if (
            !acc.find((item) => item?.serviceName === currentv?.serviceName)
          ) {
            acc.push(currentv);
          }
          return acc;
  ...
  return (
    <div className="main-service-page">
      ...
      <div>
        {/* <h1 className='py-8 text-center text-5xl  '>Our Services</h1> */}
        <div className="flex justify-center my-5">
          <p className="text-center w-11/12">
            From the amazing world of Salon services, we offer the following
            services to you at competitive prices. We have selected only the
            best for you, do check out our deals and offers of the day to avail
            the best. Availing the offers and services is simple, just Add the
            services you want to avail for a pampered day and check those
            services out. Post checking out you would be receiving an email with
      ...
      <div className="w-full flex justify-center  items-center mt-8  ">
        <div className="service-element w-11/12 ml-4 flex flex-row gap-8 justify-start flex-wrap">
          {services_provide.map((item, index) => (
            <Sercomponents key={index} item={item} />
          ))}
        </div>
      </div>
    </div>
