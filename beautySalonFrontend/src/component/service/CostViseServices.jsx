            ...
            <div className=" flex justify-center">
                <div className=" lg:w-11/12 md:w-11/12 sm:w-11/12   lg:flex lg:justify-between md:flex md:justify-between sm:flex sm:justify-between">
                    ...
                    <div className='block lg:hidden md:hidden sm:hidden mt-10'>
                        <select name="Services" id="services" className=' py-2 border-none outline-none w-full ' onChange={(e) => handleFilterServices(e.target.value)}>
                            <option value="" disabled>--Select Services--</option>
                            {services_provide.map((el, index) => <option className='font-serif  active:bg-rose-700 focus:bg-rose-600' value={el.serviceName} key={index} >
                                {el.serviceName}
                            </option>)}
                        </select>
                    </div>

                    <div className='mt-20 w-full lg:w-2/3 md:w-2/3 sm:w-2/3  lg:flex lg:justify-start md:flex md:justify-start sm:flex sm:justify-start flex-wrap gap-x-5 ' >
